...
async function updateWf02CleanNodes() {
  console.log('=== ACTUALIZANDO WF02 CON NODOS DE CÓDIGO NATIVOS ===\n');
...
await fetch('https://eeykrgnwfarrljkotvmw.supabase.co/rest/v1/content_items?id=eq.' + promptContext.content_item_id, {
  method: 'PATCH',
  headers: {
    'apikey': '${SERVICE_KEY}',
    'Authorization': 'Bearer ${SERVICE_KEY}',
...
await fetch('https://eeykrgnwfarrljkotvmw.supabase.co/rest/v1/production_outbox?id=eq.' + promptContext.outbox_event_id, {
  method: 'PATCH',
  headers: {
    'apikey': '${SERVICE_KEY}',
    'Authorization': 'Bearer ${SERVICE_KEY}',
...
`;
  ...
  const operations = [
    // 1. Remover nodos antiguos
    { type: "removeNode", nodeName: "Atomic Claim content_items" },
    { type: "removeNode", nodeName: "Update Content Item Draft" },
    { type: "removeNode", nodeName: "Update Outbox Completed" },

    // 2. Agregar Atomic Claim Code node
    {
      type: "addNode",
      node: {
    ...
    // 3. Agregar Save Generated Content Code node
    {
      type: "addNode",
      node: {
        id: "save-content-node",
    ...
    // 4. Agregar Update Outbox Skipped Code node
    {
      type: "addNode",
      node: {
        id: "outbox-skipped-node",
