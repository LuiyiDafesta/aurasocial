...
async function getAtomicClaimOutput() {
  const execs = await callTool('search_workflow_executions', { workflowId: 'QiOnKIx6QxIdzWAG', limit: 2 });
  ...
  console.log('Atomic Claim output data:', JSON.stringify(execData?.data?.resultData?.runData?.['Atomic Claim content_items'], null, 2));
}

getAtomicClaimOutput();
