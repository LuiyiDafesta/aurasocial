...
async function checkConnections() {
  const hist = await callTool('get_workflow_history', { workflowId: 'QiOnKIx6QxIdzWAG' });
  const parsed = JSON.parse(hist?.result?.content?.[0]?.text || '{}');
  const activeVerId = parsed.versions?.[0]?.versionId;
  const ver = await callTool('get_workflow_version', { workflowId: 'QiOnKIx6QxIdzWAG', versionId: activeVerId });
  const fullWf = JSON.parse(ver?.result?.content?.[0]?.text || '{}');

  console.log('Conexiones en WF02:', JSON.stringify(fullWf.connections, null, 2));
}
