...
async function testHelpersHttpRequest() {
  const claimJs = `
...
const patchRes = await this.helpers.httpRequest({
  method: 'PATCH',
  url: 'https://eeykrgnwfarrljkotvmw.supabase.co/rest/v1/content_items?id=eq.' + contentItemId + '&status=eq.queued',
  headers: {
    'apikey': '${SERVICE_KEY}',
...
const isWinner = Array.isArray(patchRes) && patchRes.length > 0;
...
await this.helpers.httpRequest({
  method: 'PATCH',
  url: 'https://eeykrgnwfarrljkotvmw.supabase.co/rest/v1/production_outbox?id=eq.' + claimData.outbox_event_id,
  headers: {
    'apikey': '${SERVICE_KEY}',
    'Authorization': 'Bearer ${SERVICE_KEY}',
    'Prefer': 'return=representation',
...
`;
  ...
  const updateRes = await callTool('update_workflow', {
    ...
    versionName: 'Uso de this.helpers.httpRequest en nodos de código'
  });
