...
async function checkBrandsRls() {
  console.log('=== CHECKING RLS POLICIES ON BRANDS ===\n');

  // Let's test using an authenticated user session!
  // First, find the user
  const { data: users } = await httpReq(`${SUPABASE_URL}/auth/v1/admin/users`);
  console.log('Users found:', users.users.map(u => ({ id: u.id, email: u.email })));

  const testUser = users.users[0];
  console.log('Testing with user:', testUser.email);

  // Generate a token or login for test user
  // Or check workspace_members
  const { data: members } = await httpReq(`${SUPABASE_URL}/rest/v1/workspace_members?select=*`);
  console.log('Workspace members:', members);
}

checkBrandsRls().catch(console.error);
