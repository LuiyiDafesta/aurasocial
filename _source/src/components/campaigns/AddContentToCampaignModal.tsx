...
import { 
  getAvailableContentsForCampaign, 
  bulkAssignContentsToCampaign 
...
interface AddContentToCampaignModalProps {
  isOpen: boolean;
  onClose: () => void;
  campaign: Campaign;
  onContentsAssigned: () => void;
}

export function AddContentToCampaignModal({
  isOpen,
  onClose,
  campaign,
  onContentsAssigned,
}: AddContentToCampaignModalProps) {
  const [contents, setContents] = useState<(ContentItem & { campaigns?: { id: string; name: string } | null })[]>([]);
  ...
  const loadContents = useCallback(async () => {
    ...
    try {
      setIsLoading(true);
      const res = await getAvailableContentsForCampaign({
      ...
      console.error('Error al cargar contenidos disponibles:', err);
      toast('Error al cargar contenidos disponibles', { type: 'error', description: err.message });
    } finally {
      setIsLoading(false);
    }
  }, [campaign.brand_id, campaign.id, searchTerm, selectedPlatform, selectedFormat, page, toast]);
  ...
  const handleSelectAllOnPage = () => {
    ...
    if (allSelected) {
      setSelectedContentIds((prev) => prev.filter((id) => !pageIds.includes(id)));
    } else {
      setSelectedContentIds((prev) => Array.from(new Set([...prev, ...pageIds])));
    }
  };

  const handleConfirmAssign = async () => {
    if (selectedContentIds.length === 0 || isSubmitting) return;

    try {
      setIsSubmitting(true);
      await bulkAssignContentsToCampaign(selectedContentIds, campaign.id);
      toast(
        `${selectedContentIds.length} ${selectedContentIds.length === 1 ? 'contenido agregado' : 'contenidos agregados'} a la campaña`,
        { type: 'success' }
      );
      onContentsAssigned();
      onClose();
    } catch (err: any) {
      console.error('Error al asignar contenidos a campaña:', err);
      toast('Error al asignar contenidos', { type: 'error', description: err.message });
    } finally {
  ...
  const countWithOtherCampaign = contents.filter(
    (c) => selectedContentIds.includes(c.id) && c.campaign_id && c.campaign_id !== campaign.id
  ).length;
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-dark-950/85 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="w-full max-w-4xl bg-dark-900 border border-dark-800 rounded-3xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh] animate-in zoom-in-95 duration-200">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-base font-bold text-white flex items-center gap-2">
                Agregar Contenidos a la Campaña
              </h2>
        ...
        <div className="p-4 bg-dark-950/70 border-b border-dark-800 flex flex-wrap items-center justify-between gap-3">
          <div className="relative flex-1 min-w-[200px]">
            ...
            <input
              ...
              placeholder="Buscar por título, hook o caption..."
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {contents.length > 0 && (
              ...
              >
                {contents.every((c) => selectedContentIds.includes(c.id))
                  ? 'Deseleccionar todo'
                  : 'Seleccionar página'}
              </Button>
            )}
        ...
        {/* Warning if moving contents from other campaigns */}
        {countWithOtherCampaign > 0 && (
          <div className="px-6 py-3 bg-amber-500/10 border-b border-amber-500/20 text-xs text-amber-300 flex items-center gap-2">
            ...
            <span>
              {countWithOtherCampaign} de los contenidos seleccionados ya pertenecen a otra campaña. Al confirmar, se moverán a <strong>{campaign.name}</strong>.
            </span>
          </div>
        )}
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-3">
          {isLoading ? (
            <div className="p-12 flex flex-col items-center justify-center space-y-3">
              ...
              <p className="text-xs text-slate-400">Consultando contenidos disponibles...</p>
            </div>
          ) : contents.length === 0 ? (
            <div className="p-12 text-center rounded-2xl bg-dark-950/50 border border-dark-800 space-y-2">
              ...
              <p className="text-xs text-slate-400">
                Todos los contenidos de la marca ya pertenecen a esta campaña o no coinciden con los filtros aplicados.
              </p>
            </div>
          ) : (
            contents.map((item) => {
              const isSelected = selectedContentIds.includes(item.id);
              const otherCampaignName = item.campaigns?.name;
              const isVideo = item.content_type?.toLowerCase().includes('video') || item.content_type?.toLowerCase().includes('reel');

              return (
                <div
                  key={item.id}
                  onClick={() => toggleSelectContent(item.id)}
                  className={cn(
                ...
                >
                  ...
                  <div className="flex-1 min-w-0 space-y-1.5">
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      {item.content_type && (
                        <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md text-[10px] font-semibold uppercase bg-dark-900 border border-dark-800 text-slate-300">
                          {isVideo ? <Film className="w-3 h-3 text-pink-400" /> : <ImageIcon className="w-3 h-3 text-sky-400" />}
                          {item.content_type}
                        </span>
                      )}
        ...
        <div className="p-4 border-t border-dark-800 bg-dark-900/90 flex items-center justify-between gap-4">
          <span className="text-xs text-slate-400">
            {selectedContentIds.length} {selectedContentIds.length === 1 ? 'contenido seleccionado' : 'contenidos seleccionados'} de {total}
          </span>
          ...
          <div className="flex items-center gap-2">
            ...
            >
              {selectedContentIds.length > 0
                ? `Agregar ${selectedContentIds.length} ${selectedContentIds.length === 1 ? 'contenido' : 'contenidos'} a la campaña`
